...
const HostTrips = () => {
  ...
  const TripsTable = ({ hostTrips, hostVehicles }) => {
    return (
      <Table variant="striped" color="black" colorScheme="blue">
        ...
        <Tbody>
          {hostTrips &&
            Object.keys(hostTrips).map((tripId, idx) => {
              let vehicle = hostVehicles[hostTrips[tripId].vehicleId];
              let trip = hostTrips[tripId];
              return (
                <Tr key={tripId}>
                  <Td pr={0}>
                    <Button>
                  ...
                  <Td>
                    {trip.startDate.slice(5).replace("-", "/")} to{" "}
                    {trip.endDate.slice(5).replace("-", "/")}
                  </Td>
                  <Td>days</Td>
                  <Td>${trip.earnings}</Td>
                  <Td>${trip.reinbursements}</Td>
                  <Td>{trip.milesDriven} mi.</Td>
                  <Td></Td>
                </Tr>
              );
            })}
  ...
  return (
    <Flex flexDirection="column" pt={20}>
      ...
      <Heading color="black">your trips.</Heading>
      <Box flexDir="column">
        <Flex flexDir="row" justify="space-between">
          <Box flexDir="row">{/* <Button>Test</Button> */}</Box>
          <Box flexDir="row">
